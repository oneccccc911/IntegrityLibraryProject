...
<!-- 스미스드롭박스 /시작 -->
                ...
                <li class="dropdown">
                    ...
                    <ul class="dropdown-menu">
                         <li>
                            <a href="#" onclick='call("${user}");'><i class="fa fa-fw fa-user"></i> Profile</a>
                                <script>
                                  function call(user){     
...
<script> 	
	...
	function callupdate(checkout_num, return_date, extend_num){
		if ((return_date.trim() != "") || (extend_num == 2)){
				alert("연장이 불가합니다.");
		}else{
			
			var result = confirm("연장하시겠습니까?");
			if(result){
				
				location.href="checkUpdate?checkout_num="+ checkout_num;
	...
	<div class="panel panel-default">
         ...
         <h3 class="panel-title"><i class="fa fa-money fa-fw"></i> 도서 대출 현황</h3>
         </div>
         ...
         <c:forEach items="${checklist}" var="check">
			<tr>
				<th>${check.checkout_num }</th>
				<td>${check.book_name }</td>
				<td>${check.checkout_date}</td>
				<td>${check.due_date }</td> 
				<td>${check.return_date }</td>
				<td>${check.extend_num } /2</td>
				<td>${check.book_ID }</td>
				<td>${check.copy_ID }</td>

				<td><button onclick="callupdate(${check.checkout_num},'${check.return_date}',${check.extend_num})">연장</button></td>
			</tr>
		  </c:forEach>
         </tbody>
     	</table>
