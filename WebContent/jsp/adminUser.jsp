...
<!-- 헤드부분추가 /시작 -->
<script>
	function calldelete(user_ID){
		var result = confirm("정말로 삭제하시겠습니까?");
		if(result){
			location.href="DeleteUser?user_ID="+user_ID;
		}else{
			alert("삭제가 취소되었습니다.");
		}
	}
...
<!-- 스미스드롭박스 /끝 -->
 ...
 <!-- 마페&manageuser /시작 -->	   
 					<c:if test="${user_ID != 'admin'}">	                 
	                    <li>
	                    <a href="#" onclick='dclick("${user}");'><i class="fa fa-fw fa-user"></i> My Page</a>
				            <script>
				                   	function dclick(user){     
...
<!-- 마페&manageuser /끝 -->
		...
		<div class="container" style="padding: 80px 200px 260px 10px">
		<h2 style="font-size: 200%;">Manage User</h2>
		
		<div class="form-horizontal" >
		    <div class="form-group row">
			
			<div style="padding: 20px 10px 5px 20px";>
			<div  style="display:inline-block;width:180px">
			      <select class="form-control" id="search" >
					...
					<option value="univ_name">학교명</option>
					<option value="univ_num">학번</option>
			      </select>
			</div>	  	      	
		 
		      		<input id="keyword"  class="form-control"  type="text" name="keyword" autofocus="autofocus" 
		      		style="display:inline-block;width:400px;height:30px"
		 			placeholder="검색어를 입력하세요."  onkeypress="enter();">
		       	
					<button class="btn btn-default" type="submit" onclick="call_search()" 
					style="display:inline-block;width:40px;height:30px;margin: 0px 0px 3.8px 0px"
		        	id="button"><i class="glyphicon glyphicon-search"></i></button> 
	...
	function call_search(){
		...
		location.href="UserSearchServlet?search="+search + "&keyword="+keyword;
	}
...
<div class="panel panel-default">                           
                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered table-hover table-striped" >                                   	
                                        ...
                                        <c:forEach items="${userlist }" var="user">
                                            <tr>
                <td><a href="MyPage2?user_ID=${user.user_ID }"> ${user.user_ID }</a></td> 
				...
				<td>${user.univ_num }</td>																																							
				<td><button id="delete" type="button" class="btn btn-default" onclick="calldelete('${user.user_ID }')">삭제</button></td>                                                
                                            </tr>                                      
                                            </c:forEach>
                                        </tbody>
                                    </table>
